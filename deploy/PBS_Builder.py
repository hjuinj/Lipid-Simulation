...
class PBS_Builder(object):
    ...
    def make_tpr(self, cfg, mpi = True, dir = None):
        if dir is None:
            dir = self.dir
    ...
    def make_mdrun(self, mpi = True, extra_args = ""):
...
class US_PBS_Builder(PBS_Builder):
    ...
    def make_us_commands(self, **kwargs):
        output = []
        for i in [self.pattern.replace("#", str(i)) for i in self.frames]:
        #    print self.make_commands(i)
            output.append(self.make_commands(i, extra_args = "-pf pullf.xvg -px pullx.xvg"))
            self.current_stage = 0
        return output
...
class RE_PBS_Builder(PBS_Builder):
    ...
    def make_tpr(self, mdp, cfg = "init", mpi = True, dir = None):
        if dir is None:
            dir = self.dir
        cmd = ""
